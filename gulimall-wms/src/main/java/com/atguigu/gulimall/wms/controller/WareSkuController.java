...
public class WareSkuController {
    ...
    @ApiOperation("pms的调用某个sku的库存信息")
    @GetMapping("/sku")
    public Resp<List<SkuStockVo>> skuInfoById(
            @RequestParam("skuId") Long skuId){
        List<WareSkuEntity> skuEntities = wareSkuService.list(new QueryWrapper<WareSkuEntity>().eq("sku_id", skuId));
        List<SkuStockVo> list = new ArrayList<>();
        skuEntities.forEach((skuEntity)->{
            SkuStockVo skuStockVo = new SkuStockVo();
            BeanUtils.copyProperties(skuEntity,skuStockVo);
            list.add(skuStockVo);
        });
        return Resp.ok(list);
    }
    ...
    public Resp<List<WareSkuEntity>> skuInfo(
