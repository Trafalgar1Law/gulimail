...
public class AttrController {
    ...
    public Resp<PageVo> pmsBaseAttr(
            QueryCondition queryCondition,
            @PathVariable(value = "catId",required = true)
            Integer catId){
        PageVo pageVo=attrService.queryAllBaseAttrByCatId(queryCondition,catId);
        return Resp.ok(pageVo);
    }

    @ApiOperation("查询某个三级分类下的所有销售属性")
    @GetMapping("/sale/{catId}")
    public Resp<PageVo> pmsSaleAttr(
            QueryCondition queryCondition,
            @PathVariable("catId") Integer catId
            ){
        PageVo pageVo=attrService.queryAllSaleAttrByCatId(queryCondition,catId);
        return Resp.ok(pageVo);
    }
//==============================================================================================
    ...
    public Resp<AttrWithGroupVo> info(@PathVariable("attrId") Long attrId){
        ...
        AttrGroupEntity attrGroupEntity=attrAttrgroupRelationService.getGroupInfoByAttrId(attrId);
